...
export default {
  ...
  methods: {
    _fetchDataForToday: function () {
      this.axios.get('https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,ETH,LTC&tsyms=BRL').then(res => {
          localStorage.setItem('BTC', this.currentCurrency.BTC = res.data.BTC.BRL),
          localStorage.setItem('ETH', this.currentCurrency.ETH = res.data.ETH.BRL),
          localStorage.setItem('LTC', this.currentCurrency.LTC = res.data.LTC.BRL)
      })
    },
    _fetchDataFor: function(key, daysAgo) {
      var self = this
      var date = this.$moment().subtract(daysAgo, 'days').unix()
      let fetch = (curr, date) => this.axios.get(`https://min-api.cryptocompare.com/data/pricehistorical?fsym=${curr}&tsyms=BRL&ts=${date}`)
      this.axios
          .all([fetch('BTC', date), fetch('ETH', date), fetch('LTC', date)])
          .then(this.axios.spread((BTC, ETH, LTC) => {
              this.previousCurrency[key] = {
                  BTC: BTC.data.BTC.BRL,
                  LTC: LTC.data.LTC.BRL,
                  ETH: ETH.data.ETH.BRL,
  ...
  created() {
    if ( ! navigator.onLine) {
      ...
      channel.bind('coin-updates', data => {
        this.currentCurrency = {BTC: data.coin.BTC.BRL, ETH: data.coin.ETH.BRL, LTC: data.coin.LTC.BRL}
      });
    }
  }
}
